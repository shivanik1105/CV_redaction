    ...
    try:
        ...
        for intel in candidates:
            ...
            try:
                ...
                critical_cov_raw = ranked.get('critical_skill_coverage')
                critical_cov = (
                    float(critical_cov_raw)
                    if isinstance(critical_cov_raw, (int, float))
                    else None
                )
                ...
                if critical_required and (critical_cov is None or critical_cov < min_required_coverage):
                    continue
        ...
        # Mirror /api/quick-search sorting
        matches.sort(
            key=lambda x: (
                x['semantic_score'],
