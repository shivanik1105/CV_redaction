...
def _is_old_style_masked_pdf(pdf_path: Path) -> bool:
    """Detect cached PDFs generated by older code that still contain [REDACTED_*] tags."""
    try:
        ...
        for page in doc:
            ...
            if len(text) > 3000:
                break
        doc.close()
        return "[REDACTED" in text or "MASKED OUTPUT (" in text
    except Exception:
        return False
...
def _run_llm_with_rate_limit(extractor, redacted_text: str, job_description: Optional[str], source_name: str, trust_source: bool = False):
...
def process_redacted_cv_text(
    ...
    with cache_lock:
        if not trust_source and not is_cv_anonymized(redacted_text):
            return {
                'success': False,
...
def process_source_cv(
    ...
    result = process_redacted_cv_text(
        ...
        trust_source=(redaction_mode == 'masked_pdf')
    )
    # Show a clean preview in the UI (no visible [REDACTED_*] markers).
    result['preview'] = cleanse_redacted_tags(redacted_text)
...
def download_redacted_zip():
    ...
    try:
        ...
        for anonymized_id in normalized_ids:
            ...
            #    If it was generated by old code and still contains [REDACTED_*] tags, delete and regenerate.
            if cached_pdf_by_id.exists() and not _is_old_style_masked_pdf(cached_pdf_by_id):
            ...
            if cached_pdf_by_id.exists():
                try:
                    cached_pdf_by_id.unlink()
                    logger.info(f"Removed old-style cached PDF for {anonymized_id}, will regenerate")
                except Exception:
                    pass
